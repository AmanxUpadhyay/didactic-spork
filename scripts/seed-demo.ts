...
async function cleanup() {
  ...
  console.log(
    `[seed]   Found ${ids.length} existing demo user(s): ${ids.join(", ")}`,
  );
...
async function createUsers(): Promise<{ amanId: string; muktaId: string }> {
  ...
  const { data: amanAuth, error: amanErr } = await admin.auth.admin.createUser({
    email: AMAN_EMAIL,
...
async function createSprintRow(cfg: SprintConfig): Promise<string> {
  ...
  if (error)
    throw new Error(
      `Failed to create sprint (week ${cfg.weeksAgo} ago): ${error.message}`,
    );
  return data.id;
}
...
async function createAppreciationNotes(
  ...
  if (error)
    throw new Error(`Failed to create appreciation_notes: ${error.message}`);
}
...
async function createSprints(
  ...
  const pun1Id = await createPunishment(sprint1Id, muktaId, amanId, {
    datePlan: {
      ...
      activity_type: null,
      notes:
        "Dinner for two at Balans Soho, Old Compton Street. 3-course Italian menu.",
    },
  ...
  await createPunishment(sprint2Id, amanId, muktaId, {
    datePlan: {
      ...
      notes:
        "Intermediate Italian cooking class at COOK, Covent Garden. 3-hour session.",
    },
...
async function createStreaks(
  ...
  console.log(
    `[seed]   Streaks: ${amanStreaks.length + muktaStreaks.length + 1} rows`,
  );
}

// ─── Notification Preferences ──────────────────────────────────────────────

async function createNotificationPreferences(amanId: string, muktaId: string) {
...
async function createMoodEntries(amanId: string, muktaId: string) {
  ...
  if (error) throw new Error(`Failed to create mood_entries: ${error.message}`);
...
async function main() {
  ...
  try {
    ...
    console.log(
      "\nSee docs/operations/demo-credentials.md for full scenario details.\n",
    );
  } catch (err) {
    console.error("\n[seed] FAILED:", err);
    process.exit(1);
